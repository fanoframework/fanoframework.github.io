...
If it is not set, by default, code above will use GUID as session id (using `TGuidSessionIdGenerator` class). You can change the way session id generated by calling `sessionIdGenerator()` method and pass factory class of `ISessionIdGeneratorFactory` interface, as shown in following code,
